...
Window::Window(HWND hWnd, const std::wstring& windowName, int clientWidth, int clientHeight, bool vSync)
    ...
    for(int i = 0; i < BufferCount; ++i) {
        m_pBackBufferTextures[i]->SetName(L"Backbuffer[" + std::to_wstring(i) + L"]");
    }
...
void Window::UpdateRenderTargetViews() {
    for(int i = 0; i < BufferCount; ++i) {
        ...
        m_pBackBufferTextures[i] = std::make_shared<Texture>(Texture(backBuffer));
    }
}
...
const RenderTarget& Window::GetCurrentRenderTarget() const {
    m_RenderTarget.AttachTexture(AttachmentPoint::Color0, m_pBackBufferTextures[m_CurrentBackBufferIndex]);
    return m_RenderTarget;
}

UINT Window::Present(const std::shared_ptr<Texture>& pTexture = nullptr) {
    auto commandQueue = Application::Get().GetCommandQueue(D3D12_COMMAND_LIST_TYPE_DIRECT);
    auto commandList = commandQueue->GetCommandList();

    auto pBackBuffer = m_pBackBufferTextures[m_CurrentBackBufferIndex];

    if(pTexture) {
        if(pTexture->GetD3D12ResourceDesc().SampleDesc.Count > 1) {
            commandList->ResolveSubresource(pBackBuffer, pTexture);
        }
        else {
            commandList->CopyResource(pBackBuffer, pTexture);
        }
    }

    RenderTarget renderTarget;
    renderTarget.AttachTexture(AttachmentPoint::Color0, pBackBuffer);

    //m_GUI.Render(commandList, renderTarget);

    commandList->TransitionBarrier(pBackBuffer, D3D12_RESOURCE_STATE_PRESENT);
